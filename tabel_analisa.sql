...
FROM (
  ...
  FROM 
    `rakamin-kf-analytics-448111.Kimia_farma.kf_final_transaction` ft
    JOIN `rakamin-kf-analytics-448111.Kimia_farma.kf_kantor_cabang` kc ON ft.branch_id = kc.branch_id
    JOIN `rakamin-kf-analytics-448111.Kimia_farma.kf_product` p ON ft.product_id = p.product_id
  WHERE 
